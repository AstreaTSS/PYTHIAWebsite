---
description: A handful of guides to teach you how to set up the features of PYTHIA.
---

# Server Setup Guides

Below are a handful of guides to teach you how to set up the features of PYTHIA. You should read the basic setup guide first, but from there, feel free to look at the features you want in your RP!

## Guides
- [Basic Setup](basic_setup.md)
- [BDA Investigations Setup](bda_investigations_setup.md)
- [Dice Management](dice_management.md)
- [Gacha Setup](gacha_setup.md)
- [Items Setup](items_setup.md)
- [Messaging Setup](messaging_setup.md)
